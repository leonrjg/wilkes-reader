...
export function Tooltip({ content, children, className = "", delayMs = 0, interactive = false, size = "default", }) {
    const id = useId();
    ...
    const closeFromInteraction = () => {
        ...
        closeTimerRef.current = setTimeout(() => {
            closeTimerRef.current = null;
            if (!triggerPointerInsideRef.current
    ...
    useLayoutEffect(() => {
        ...
        const margin = 8;
        const gap = 6;
        const x = Math.min(Math.max(triggerRect.left + triggerRect.width / 2 - tooltipRect.width / 2, margin), window.innerWidth - tooltipRect.width - margin);
    ...
    return (_jsxs(_Fragment, { children: [React.cloneElement(child, {
                ...
                createPortal(_jsx("div", { ref: tooltipRef, id: id, role: "tooltip", className: `${interactive ? "pointer-events-auto select-text" : "pointer-events-none"} fixed z-[180] ${size === "wide" ? "max-w-[440px]" : "max-w-[320px]"} rounded border border-[var(--border-main)] bg-[var(--bg-app)] px-2 py-1 text-[11px] leading-snug text-[var(--text-main)] shadow-xl ${className}`, style: {

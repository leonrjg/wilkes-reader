...
function isDiscretionaryHyphen(value) {
    return (value === "-" ||
        value === "\u00ad" ||
        value === "\u2010" ||
        value === "\u2011");
}
function isIgnoredFormatCharacter(value) {
    return (value === "\u200b" ||
        value === "\u200c" ||
        value === "\u200d" ||
        value === "\u2060" ||
...
function normalize(units, trimOuterWhitespace) {
    const normalized = [];
    let index = 0;
...
function literalMatcher(query, caseSensitive) {
    ...
    for (const [index, unit] of units.entries()) {
        ...
        const nextIsLetter = index + 1 < units.length && isLetter(units[index + 1].value);
...
function rectForFragment(item, start, end) {
    if (!item.text || end <= start)
        return null;
    if (!item.horizontal) {
...
function commonSuffixLength(left, right, limit = 120) {
    let length = 0;
    while (length < limit &&
        length < left.length &&
        length < right.length &&
...
function commonPrefixLength(left, right, limit = 120) {
    let length = 0;
    while (length < limit &&
        length < left.length &&
        length < right.length &&
...
function normalizedString(input) {
    return normalize(stringUnits(input), false).map((unit) => unit.value).join("");
}
function matchRects(items, projection, start, end) {
    const fragments = new Map();
    for (const span of projection.spans) {
        if (span.start >= end || span.end <= start || span.itemIndex === null)
            continue;
    ...
    return mergeRectsByLine([...fragments.entries()].flatMap(([itemIndex, fragment]) => {
        const rect = rectForFragment(items[itemIndex], fragment.start, fragment.end);
        return rect ? [rect] : [];
    }));
}
export function findPdfTextMatchesInItems(items, page, query, options = {}) {
 ...
 */
export async function locatePdfSearchResult(pdf, coarsePage, locator, signal) {
    const offsets = [0, 1, -1, 2, -2];
    const pages = offsets
        .map((offset) => coarsePage + offset)
        .filter((page, index, all) => page >= 1 && page <= pdf.numPages && all.indexOf(page) === index);
    const candidates = [];
    ...
    candidates.sort((left, right) => right.contextScore - left.contextScore || left.pagePriority - right.pagePriority);
    const best = candidates[0];
    if (signal?.aborted || !best)
        return null;
    const { pagePriority: _pagePriority, ...match } = best;
    return match;
}
//# sourceMappingURL=pdfTextLocator.js.map
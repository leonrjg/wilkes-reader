...
export function getScaledPageHeight(metric, renderedWidth) {
    return (metric.height / metric.width) * renderedWidth;
}
export function usePdfPageMetrics(pdf, url) {
    ...
    useEffect(() => {
        ...
        const loadMetrics = async () => {
            try {
                const metrics = await Promise.all(Array.from({ length: pdf.numPages }, async (_, index) => {
...
//# sourceMappingURL=usePdfPageMetrics.js.map
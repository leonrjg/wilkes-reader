 ...
 */
export function renderedBoundaries(source, rendered, start, end) {
    const slice = source.slice(start, end);
    const boundaries = new Array(rendered.length + 1).fill(start);
    let sourceOffset = 0;
...
function annotationKey(start, end, annotations) {
    return annotations
...
export function sourceMappedMarkdown(content, annotations) {
    const annotationsById = new Map(annotations.map((annotation) => [annotation.id, annotation]));
    const sourceByteBoundaries = new Array(content.length + 1).fill(0);
    let utf16Offset = 0;
    let byteOffset = 0;
    ...
    return () => (tree) => {
        const visit = (node, inPre) => {
            ...
            node.children = node.children.flatMap((child) => {
                ...
                const boundaries = renderedBoundaries(content, value, child.position.start.offset, child.position.end.offset);
                ...
                const runs = [];
                let runStart = 0;
                let key = annotationKey(byteBoundaries[0], byteBoundaries[characterBoundaries[1] ?? 0], annotations);
                    ...
                    if (childInPre) {
                        ...
                        for (let offset = runStart; offset < index; offset += 1) {
                            ...
                            if (offset > segmentStart)
                                runs.push(span(segmentStart, offset));
                            runs.push({ type: "element", tagName: "br", properties: {}, children: [] });
                            segmentStart = offset + 1;
                        }
                        if (segmentStart < index)
                            runs.push(span(segmentStart, index));
                    }
                    runStart = index;
                    key = nextKey ?? "|";
                }
...
export function sourceBoundaryForDomPoint(node, offset) {
    const element = (node instanceof Element ? node : node.parentElement)?.closest(".markdown-source-run");
    if (!element)
        return null;
    const boundaries = element.dataset.sourceBoundaries?.split(",").map(Number);
    if (!boundaries || boundaries.some(Number.isNaN))
        return null;
...
//# sourceMappingURL=markdownSourceMap.js.map